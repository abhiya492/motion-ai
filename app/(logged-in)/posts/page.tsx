...
export default async function Page() {
  ...
  return (
    <main className="mx-auto w-full max-w-screen-xl px-2.5 lg:px-0 mb-12 mt-28">
      ...
      {posts.length === 0 && (
        <div>
          <p className="text-gray-700 text-lg lg:text-xl mb-4 line-clamp-3">
            You have no posts yet. Upload a video or audio to get started.
          </p>
          <Link
            href={`/dashboard`}
            className="text-purple-700 hover:text-purple-800 font-medium flex gap-2 items-center"
          >
        ...
        {posts.map((post) => (
          <BgGradient key={post.id}>
            <div className="bg-white shadow-md rounded-lg p-6 hover:shadow-lg transition-shadow duration-300">
              ...
              <Link
                href={`/posts/${post.id}`}
                className="text-purple-700 hover:text-purple-800 font-medium flex gap-1 items-center"
              >
